...
function mouseMove1(e) {
  if (firefox) {
    ...
    chuot.x = event.clientX;
    chuot.y = event.clientY;
  }
}
...
function pre_load_img(ar, callback) {
  this.ar = Array();
  m = ar.length;
  for (i = 0; i < m; i++) {
    this.ar[i] = ar[i];
  ...
  this.load_item = function (stt, arr) {
    if (stt < this.count) {
      this.ar[stt] = new Image();
      this.ar[stt].src = arr[stt];
      this.ar[stt].onload = this.load_item(stt + 1, arr);
    } else {
      eval(callback);
    }
  };
...
function hilight_text(str, ar, c) {
  str2 = "";
...
function strip_tags(str) {
  var t = str.replace(
    /<(\/)?(html|head|title|body|h1|h2|h3|h4|h5|h6|p|br|hr|pre|em|strong|code|b|i|a|ul|li|ol|dl|dd|table|tr|t  h|td)([^>]*)>/gi,
    ""
  );
  t = t.replace(
    /<(\/)?(iframe|frameset|form|input|select|option|textarea|blackquote|address|object)([^>]*)>/gi,
    ""
  );
  return t;
}
